...
class BVAnalysis(object):
    ...
    def SaveBVSEData(self, filename):
        ...
        self.SaveBinaryData('mid_out/' + 'Lai_Na/' + filename.replace(".cif", "").split('\\')[1] + '_BVSE.pgrid', self.__Data['BVSE'],
                            'BVSE')
        np.save('mid_out/' + 'Lai_Na/' + filename.replace(".cif", "").split('\\')[1] + "_BVSE.npy", self.__Data['BVSE'])
