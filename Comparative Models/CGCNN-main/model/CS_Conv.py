...
class Node_Convolution(torch.nn.Module):
    def __init__(self, args):
        ...
        self.message_linear = Linear(args.nhid * 2, args.nhid, bias=True)
        self.node_linear = Linear(args.nhid * 2, args.nhid, bias=True)
    def forward(self, x, edge_attr, edge_source, edge_target):
        comb = torch.cat([x[edge_source], x[edge_target]], dim=-1)
        comb = F.relu(self.message_linear(comb))
        node_message = scatter_mean(comb, edge_source, dim=0)
        # print("x: ", x.shape)
        # print("node_message: ", node_message.shape)
        x = torch.cat([x, node_message], dim=-1)
...
class CS_Conv(torch.nn.Module):
    def __init__(self, args):
        super(CS_Conv, self).__init__()
        self.node_embedding = Sequential(
            Linear(args.cs_x_features_dim, args.nhid, bias=True), ReLU()
        )
        self.edge_embedding = Sequential(
            Linear(args.cs_edge_features_dim, args.nhid, bias=True), ReLU()
        )
