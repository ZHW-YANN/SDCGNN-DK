...
def cal_CGCNN_train(loader):
    ...
    for input, targets, ids in loader:
        ...
        out = cs_model(cs_x, cs_edge_sources, cs_edge_targets, cs_edge_attr, cs_node_batch)
...
def cs_collate(batch):
    ...
    cs_node_batch = []
    cs_total_count = 0
    for i, (cs_x, cs_edge_index, cs_edge_attr, targets, ids) in enumerate(batch):
    ...
    return (batch_cs_x, batch_cs_edge_source, batch_cs_edge_target, batch_cs_edge_attr, cs_node_batch), batch_targets, batch_ids


if __name__ == '__main__':
    args.dataset_path = args.dataset
    dataset = load_data.load_data_CGCNN(args.dataset_path)
    for i in range(1, 11):
        args.seed = i
        ...
        train_loader = DataLoader(train_set, batch_size=args.batch_size, shuffle=True, collate_fn=cs_collate)
        val_loader = DataLoader(val_set, batch_size=args.batch_size, shuffle=False, collate_fn=cs_collate)
        test_loader = DataLoader(test_set, batch_size=args.batch_size, shuffle=False, collate_fn=cs_collate)
        ...
        cs_model = CGCNN_Conv.CS_CGCNN(args).to(args.device)
        cs_optimizer = torch.optim.Adam(cs_model.parameters(), lr=args.lr, weight_decay=args.weight_decay1)
        cs_scheduler = torch.optim.lr_scheduler.StepLR(cs_optimizer, step_size=60, gamma=0.1)
        t = time.time()
        ...
        torch.save(cs_model, f'save_model/CGCNN{i}.pth')
