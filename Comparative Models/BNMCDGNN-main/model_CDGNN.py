...
from torch.nn import (Linear, Sigmoid, Softplus, ELU, ReLU, BatchNorm1d, ModuleList, Sequential, Tanh)
		...
		for input, targets in dataloader:
			...
			with torch.set_grad_enabled(False):
				outputs, graph_vec = self.model(nodes, edge_sources, edge_targets, edge_distance, graph_indices,
												node_counts, combine_sets, output_graph=True)
...
class GatedGraphConvolution(Module):
	def __init__(self, n_node_feat, in_features, out_features, n_Gaussian,
				 gate_activation=Sigmoid(),
				 use_node_batch_norm=False, use_edge_batch_norm=True,
				 bias=True, conv_type=0, MLP_activation=ELU()):
		super(GatedGraphConvolution, self).__init__()
		k1 = n_Gaussian  # k is the number of basis
		self.linear1_vector = Linear(k1, out_features, bias=bias)  # 线性径向基
		self.linear1_vector_gate = Linear(k1, out_features, bias=bias)  # 线性径向基
	...
	def forward(self, input, nodes, edge_sources, edge_targets, rij, combine_sets, cutoff):
...
class CDGNN(nn.Module):
	def __init__(self, n_node_feat, n_hidden_feat, n_GCN_feat, conv_bias, N_block, node_activation, MLP_activation,
				 use_node_batch_norm, use_edge_batch_norm, cutoff, max_nei, n_MLP_LR, n_Gaussian):

		super(CDGNN, self).__init__()
		self.cutoff = cutoff
		...
		self.conv = [
			GatedGraphConvolution(n_node_feat, n2v_concatent_feat, n_hidden_feat, n_Gaussian,
	...
	def forward(self, nodes, edge_sources, edge_targets, edge_distance, graph_indices, node_counts, combine_sets,
		...
		for i in range(self.N_block):
			x = self.conv[i](x, nodes, edge_sources, edge_targets, edge_distance, combine_sets, self.cutoff)
