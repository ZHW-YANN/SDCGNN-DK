...
def cal_GATGNN_train(loader):
    cs_model.train()
    loss_train = 0.0
    for input, targets, ids in loader:
        cs_optimizer.zero_grad()
        cs_x = input[0].to(args.device)
        cs_edge_sources = input[1].to(args.device)
        cs_edge_targets = input[2].to(args.device)
        cs_edge_attr = input[3].to(args.device)
        cs_global_feature = input[4].to(args.device)
        cs_node_batch = input[5].to(args.device)
        targets = targets.to(args.device)
        out = cs_model(cs_x, cs_edge_sources, cs_edge_targets, cs_edge_attr, cs_global_feature, cs_node_batch)
...
def gat_collate(batch):
    batch_cs_x = []
    batch_cs_edge_source = []
    batch_cs_edge_target = []
    batch_cs_edge_attr = []
    batch_cs_global_feature = []
    batch_targets = []
    batch_ids = []
    cs_node_batch = []
    cs_total_count = 0
    for i, (cs_x, cs_edge_index, cs_edge_attr, cs_global_feature, targets, ids) in enumerate(batch):
        ...
        batch_cs_global_feature.append(cs_global_feature)
    ...
    batch_cs_global_feature = np.concatenate(batch_cs_global_feature, axis=0)
    ...
    batch_cs_global_feature = torch.Tensor(batch_cs_global_feature)
    batch_targets = torch.Tensor(batch_targets)
    cs_node_batch = torch.LongTensor(cs_node_batch)

    return (batch_cs_x, batch_cs_edge_source, batch_cs_edge_target, batch_cs_edge_attr, batch_cs_global_feature, cs_node_batch), batch_targets, batch_ids
...
if __name__ == '__main__':
    ...
    with open(config_path) as f:
        config = json.load(f)
    dataset = load_data.load_data_GATGNN(args.dataset_path, config)
    for i in range(6, 11):
        args.seed = i
        ...
        train_loader = DataLoader(train_set, batch_size=args.batch_size, shuffle=True, collate_fn=gat_collate)
        val_loader = DataLoader(val_set, batch_size=args.batch_size, shuffle=False, collate_fn=gat_collate)
        test_loader = DataLoader(test_set, batch_size=args.batch_size, shuffle=False, collate_fn=gat_collate)
        ...
        cs_model = GATGNN_Conv.GATGNN(args).to(args.device)
        cs_optimizer = torch.optim.Adam(cs_model.parameters(), lr=args.lr, weight_decay=args.weight_decay1)
        cs_scheduler = torch.optim.lr_scheduler.StepLR(cs_optimizer, step_size=50, gamma=0.5)
        t = time.time()
        for epoch in range(args.epochs):
            train_loss = cal_GATGNN_train(train_loader)
            val_loss, r2_val = cal_GATGNN_eval(val_loader)
            test_loss, r2_test = cal_GATGNN_eval(test_loader)
            cs_scheduler.step()
