...
class GeoCGNN(torch.nn.Module):
    def __init__(self, args):
        ...
        self.lin1 = OLP(int(self.n_hidden_feat / 2 ** (self.n_MLP_LR - 1)), 1, activation=None, use_batch_norm=None, bias=args.conv_bias)


    def forward(self, x, edge_sources, edge_targets, edge_distance,
                graph_indices, node_counts, combine_sets, plane_wave,
                output_graph=False):
        x = self.embedding(x)
        PoolingResults = []
