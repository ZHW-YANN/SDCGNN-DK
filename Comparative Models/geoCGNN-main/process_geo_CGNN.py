...
def main(device, model_param, optimizer_param, scheduler_param, dataset_param, dataloader_param,
    ...
    df_predictions = pd.DataFrame({"id": names, "prediction": outputs, "targets": targets})
    all_graph_vec=pd.DataFrame(all_graph_vec)
    all_graph_vec['name']=names
    name=str(N_block)+'_'+str(cutoff)+'_'+str(max_nei)
    df_predictions.to_csv("data/test_predictions_{}_seed2.csv".format(name), index=False)
    all_graph_vec.to_csv("data/all_graph_vec_{}_seed2.csv".format(name), index=False)
    print("\nEND")
...
if __name__ == '__main__':
    ...
    parser.add_argument("--milestones", nargs='+', type=int, default=[150])
